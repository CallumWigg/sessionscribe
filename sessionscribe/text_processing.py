...
def apply_corrections(text):
    ...
    for word in text.split():
        if word.lower() in [w.lower() for w in utils.load_custom_words()]:
            corrected_text.append(word)  # Keep original case
            continue

        phonetic_word = metaphone(word)
        if phonetic_word in utils.phonetic_dict():
            corrected_text.append(utils.phonetic_dict()[phonetic_word])
            continue
...
def fuzzy_fix():
    ...
    for incorrect in list(incorrect_words.keys()):
        if not incorrect_words[incorrect]:
            best_match, score = process.extractOne(incorrect, utils.load_custom_words(), scorer=fuzz.ratio)
    ...
    with open(utils.get_corrections_list_file(), 'w', encoding='utf-8') as f:
